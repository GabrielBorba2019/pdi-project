...
public class IngredientService {
    ...
    public IngredientService(
            ...
            IngredientTypeRepository ingredientTypeRepository, IngredientRepository ingredientRepository) {
    ...
    public List<Ingredient> createIngredientList(List<IngredientDTO> ingredientsDTO) {
        ...
        List<Long> ingredientTypesIds = ingredientsDTO.stream()
                .map(IngredientDTO::getType)
                .distinct()
                .collect(Collectors.toList());

        List<Long> unitMeasureIds = ingredientsDTO.stream()
                .map(IngredientDTO::getUnit)
                .distinct()
                .collect(Collectors.toList());
        ...
        List<Ingredient> ingredientList = ingredientsDTO.stream()
    ...
    private Ingredient mapToEntity(IngredientDTO dto, List<IngredientType> ingredientTypes, List<UnitOfMensurement> unitOfMensurements) {
        IngredientType ingredientType = ingredientTypes.stream()
                .filter(type -> type.getId().equals(dto.getType()))
                .findFirst().orElse(null);

        UnitOfMensurement unitOfMensurement = unitOfMensurements.stream()
    ...
    private void validateIngredientDTO(IngredientDTO dto, IngredientType ingredientType, UnitOfMensurement unitOfMensurement) {
        ...
        if (ingredientTypeDoesNotExistsOnDataBase)
            throw new IngredientValidationException("O tipo de ingrediente com id  " + dto.getType() + " não existe na base de dados.");

        if (isNull(dto.getType()))
            throw new IngredientValidationException("O tipo de ingrediente é obrigatório para o ingrediente " + dto.getName());

        var unitOfMensurementDoesNotExistsOnDataBase = !isNull(dto.getUnit()) && isNull(unitOfMensurement);
        if (unitOfMensurementDoesNotExistsOnDataBase)
            throw new IngredientValidationException("A unidade de medida com id  " + dto.getType() + " não existe na base de dados.");

        if (isNull(dto.getUnit()))
            throw new IngredientValidationException("A unidade de medida é obrigatória para o ingrediente " + dto.getName());
    }
}
