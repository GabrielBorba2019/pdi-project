...
class RecipeServiceTest {
    ...
    private StepInstructionService stepInstructionService = Mockito.mock(StepInstructionService.class);

    private RecipeService service = new RecipeService(recipeRepository,categoryRepository,subCategoryRepository,stepInstructionService, ingredientService);
    ...
    private final Recipe cakeRecipe = new Recipe(1L, null, MEAT_CATEGORY, PORK_SUBCATEGORY, List.of(INGREDIENT), List.of(FIRST_STEP),null, 2, null);
    ...
    void createRecipeWithoutName(){
        RuntimeException exception = assertThrows(RecipeValidationException.class, () -> {
            var recipeDTO = (mockRecipeDTO(1l, 2l , "00:40:00", mockIngredientsDTO(CARNE_TYPE,UNIDADE), mockInstructionDTO(FIRST_STEP)));
    ...
    void createRecipeWithoutCategoryId(){
        RuntimeException exception = assertThrows(CategoryInvalidExpection.class, () -> {
            service.addRecipe(mockRecipeDTO(null, 2L , "00:40:00", mockIngredientsDTO(CARNE_TYPE,UNIDADE), mockInstructionDTO(FIRST_STEP)));
        });
    ...
    void createRecipeWithoutSubtegoryId(){
        RuntimeException exception = assertThrows(CategoryInvalidExpection.class, () -> {
           var recipe = service.addRecipe(mockRecipeDTO(1L, null, "00:40:00" , mockIngredientsDTO(CARNE_TYPE,UNIDADE), mockInstructionDTO(FIRST_STEP)));
    ...
    private RecipeDTO mockRecipeDTO(Long category, Long subCategory, String prepareTime , IngredientDTO ingredientDTO, StepInstructionDTO stepInstructionDTO){
        var recipeDTO = new RecipeDTO("Bolo de cenoura" , category , subCategory, List.of(ingredientDTO), List.of(stepInstructionDTO), prepareTime, 02);
        return  recipeDTO;
    }

    private IngredientDTO mockIngredientsDTO(IngredientType ingredientType, UnitOfMensurement unitOfMensurement) {
        var ingredientDTO = new IngredientDTO("Arroz",  ingredientType.getId(), unitOfMensurement.getId(), "10");
